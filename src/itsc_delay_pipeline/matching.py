...
class AnchorVehicleParams:
    ...
    max_dt_s: float
    max_dist_m: float
    # --- OTA-calibrated additions (see PARAMETERS.md) ---
    all_stops: bool = True                 # vote with every stop, not just first/last
    small_umlauf_max_fahrten: int = 5      # size split for the confidence gate
    confident_mrel_small: float = 0.7      # min margin/top for small Umlaeufe
    confident_mrel_large: float = 0.4      # min margin/top for large Umlaeufe


def build_anchors_from_soll(soll_m: gpd.GeoDataFrame, all_stops: bool = True) -> gpd.GeoDataFrame:
    """Scheduled stops used as vehicle-assignment anchors.

    With ``all_stops=True`` (OTA default) EVERY scheduled stop of every Fahrt is
    an anchor. The earlier first/last-only scheme gave a 30-stop Fahrt the same
    voting weight as a 3-stop one, and a single coincidentally nearby vehicle at
    just the two termini could win the vote outright. Voting over all stops makes
    the assignment robust: a wrong vehicle would have to coincide at many
    intermediate anchors too, not only start and end.
    """
    ...
    for frt, grp in soll_m.groupby("frt_fid"):
        ...
        if all_stops:
            for i, (_, row) in enumerate(g.iterrows()):
                atype = "start" if i == 0 else ("end" if i == n - 1 else "mid")
                anchors.append({"frt_fid": frt, "anchor_type": atype,
                                "stop_seq": int(row["stop_seq"]),
                                "ts_soll": row["ts_soll"], "geometry": row[gc]})
        else:
...
def match_vehicles_via_anchors(soll_m: gpd.GeoDataFrame, vp_m: gpd.GeoDataFrame, params: AnchorVehicleParams):
    anchors = build_anchors_from_soll(soll_m, all_stops=params.all_stops)
    sidx = vp_m.sindex
    ...
    # --- Size-aware confidence gate (OTA-benchmarked; see PARAMETERS.md) ---
    # mrel = (top_hits - second_hits) / top_hits is the discriminator: against
    # operator ground truth, correct assignments had a median mrel of 0.92 and
    # wrong ones 0.29. Raw top_hits is NOT usable alone - it scales with Umlauf
    # size. A small Umlauf has few anchors to vote with, so the same relative
    # margin means less there; hence a higher threshold for small Umlaeufe.
    n_fahrten = int(soll_m["frt_fid"].nunique()) if len(soll_m) else 0
    top = float(agg.iloc[0]["anchor_hits"]) if len(agg) else float("nan")
    second = float(agg.iloc[1]["anchor_hits"]) if len(agg) > 1 else 0.0
    margin = (top - second) if len(agg) else float("nan")
    mrel = (margin / top) if (len(agg) and top) else float("nan")
    thr = (params.confident_mrel_small if n_fahrten <= params.small_umlauf_max_fahrten
           else params.confident_mrel_large)
    summary = {
        "vehicle_id": None if best_day_vehicle is None else str(best_day_vehicle),
        "n_fahrten": n_fahrten,
        "n_candidates": int(len(agg)),
        "top_hits": top, "second_hits": second, "margin": margin, "mrel": mrel,
        "confident": int(bool(pd.notna(mrel) and mrel >= thr)),
        "mrel_threshold": thr,
    }
    return best_day_vehicle, df_anchor, agg, summary
