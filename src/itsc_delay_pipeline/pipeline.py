...
def run_pipeline(cfg: Config) -> dict:
    ...
    # ---- GTFS-RT feed-lag correction (see PARAMETERS.md, "Feed-lag") ----
    # Rows whose feed timestamp equals the scrape time carry the feed's position
    # AGE, not a real fix time, and lag the true time-at-position by ~23 s. Only
    # those fallback rows are shifted back; rows with a genuine feed timestamp
    # already sit within ~1 s and would be pushed early by a blanket correction.
    fl = raw.get("feed_lag", {})
    fallback_lag_s = float(fl.get("fallback_lag_s", 14.3))
    if fallback_lag_s and "time_scrape" in vp.columns:
        scrape = normalize_time_utc(vp["time_scrape"], assume_local_if_naive=False, local_tz=local_tz)
        is_scrape = vp["ts"].eq(scrape)
        vp.loc[is_scrape, "ts"] = vp.loc[is_scrape, "ts"] - pd.Timedelta(seconds=fallback_lag_s)
    ...
    av_params = AnchorVehicleParams(
        ...
        all_stops=bool(av.get("all_stops", True)),
        small_umlauf_max_fahrten=int(av.get("small_umlauf_max_fahrten", 5)),
        confident_mrel_small=float(av.get("confident_mrel_small", 0.7)),
        confident_mrel_large=float(av.get("confident_mrel_large", 0.4)),
    )

    best_vehicle_auto, df_anchor, agg_vehicle, vehicle_summary = match_vehicles_via_anchors(
        soll_m, vp_m, av_params
    )
    ...
    if str(cfg.umlauf_id) in manual_map:
        ...
        vehicle_summary = {**vehicle_summary, "vehicle_id": best_vehicle_final,
                           "confident": 1, "manual_override": True}
    ...
    arrivals = compute_arrivals_anchor_interp(
        ...
        time_gate=time_gate,
        likelihood_max_dist_m=float(arr.get("likelihood_max_dist_m", 50.0)),
        sigma_dist_m=float(arr.get("sigma_dist_m", 25.0)),
        stop_anchor_dist_m=float(arr.get("stop_anchor_dist_m", 25.0)),
    ...
    # ---- Delay = actual - scheduled (seconds) ----
    if {"ankunft_ist", "ankunft_soll"}.issubset(final_stops.columns):
        ist = pd.to_datetime(final_stops["ankunft_ist"], utc=True, errors="coerce")
        soll = pd.to_datetime(final_stops["ankunft_soll"], utc=True, errors="coerce")
        final_stops["delay_s"] = (ist - soll).dt.total_seconds()
    ...
    # per-Umlauf vehicle-assignment quality (confidence gate; see PARAMETERS.md)
    summary_row = {"betriebstag": tag, "um_uid": umlauf_id, **vehicle_summary}
    assign_out = export_dir / f"vehicle_assignment_{umlauf_id}_{tag}.csv"
    pd.DataFrame([summary_row]).to_csv(assign_out, index=False)

    return {
        "best_vehicle": str(best_vehicle_final),
        "confident": int(vehicle_summary.get("confident", 0)),
        "mrel": vehicle_summary.get("mrel"),
        "csv": str(csv_out),
        "html": str(html_out),
        "anchors_csv": str(df_anchor_out),
        "vehicle_ranking_csv": str(agg_out),
        "vehicle_assignment_csv": str(assign_out),
    }
